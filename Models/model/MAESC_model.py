...
class MultiModalBartModel_AESC(PretrainedBartModel):
    ...
    def __init__(self, config: MultiModalBartConfig, args, bart_model, tokenizer, label_ids):
    ...
    def prepare_state(self,
        ...
        state = BartState(encoder_outputs,
                          encoder_mask,
                          input_ids[:,-length:],  # the text features start from index 38, the front are image features.
                          first,
                          src_embed_outputs)
    ...
    def forward(self,
        ...
        state = self.prepare_state(input_ids, image_features, attention_mask, length=sentence_length)
        spans, span_mask = [aesc_infos['labels'].to(input_ids.device), aesc_infos['masks'].to(input_ids.device)]
