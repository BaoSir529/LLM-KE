...
class MultiModalBartModelForPretrain(FromPretrainedMixin, PretrainedBartModel):
    def build_model(self,
        ...
        if args.bart_init:
            ...
            for token in tokenizer.unique_no_split_tokens:
                if token[:2] == '<<':  # 特殊字符
                    index = tokenizer.convert_tokens_to_ids(tokenizer._base_tokenizer.tokenize(token))
                    if len(index) > 1:
    ...
    def __init__(self, config: MultiModalBartConfig, bart_model, tokenizer, label_ids, senti_ids, args):
        super().__init__(config)
        self.config = config
        # label_ids = sorted(label_ids)
        multimodal_encoder, share_decoder = self.build_model(args, bart_model, tokenizer, label_ids, config)
        ...
        self.anp_generate_decoder = MultiModalBartDecoder_ANP_generate(self.config,
                                                                       share_decoder)
        self.senti_decoder = MultiModalBartDecoder_sentiment(self.config,
                                                             share_decoder,
                                                             senti_ids)
    ...
    def prepare_state(self,
        ...
        state = BartState(encoder_outputs, encoder_mask, input_ids[:, 38:], first, src_embed_outputs)
    ...
    def forward(self,
        ...
        if task_type == 'MLM':
            labels, decoder_input_ids, decoder_attention_mask = [mlm_infos['mlm_labels'], mlm_infos['mlm_decoder_input_ids'],
            ...
            mrm_labels, mrm_masks, decoder_input_ids, decoder_attention_mask = [mrm_infos['mrm_labels'],
            ...
            senti_labels, decoder_input_ids, decoder_attention_mask = [senti_infos['senti_labels'],
            ...
            labels, decoder_input_ids, decoder_attention_mask = [ANP_generate_infos['anp_generate_labels'],
            ...
            spans, span_mask = [ae_oe_infos['labels'].to(input_ids.device), ae_oe_infos['masks'].to(input_ids.device)]
            state = self.prepare_state(input_ids,
                                       image_features,
                                       attention_mask)
