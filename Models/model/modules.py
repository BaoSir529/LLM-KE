...
class MultiModalBartDecoder_span(nn.Module
    ...
    def forward(self, tokens, state, only_sc=False):
        ...
        encoder_outputs = state.encoder_output  # last hidden state
        ...
        if self.need_tag:
            tag_scores = F.linear(
                hidden_state,
                self.dropout_layer(
                    self.decoder.embed_tokens.weight[self.mapping[2:]]
                )
            )  # bsz x max_len x num_class
            # logits[:, :, 2:self.src_start_index] = tag_scores
            logits[:, :, 2:self.src_start_index] = tag_scores
        if not only_sc:
            eos_scores = F.linear(hidden_state, self.dropout_layer(self.decoder.embed_tokens.weight[2:3]))
            ...
            if first is not None:
                ...
                mask = state.encoder_mask[:, -src_tokens.size(-1):].eq(0)
                # src_outputs = self.decoder.embed_tokens(src_tokens)
            mask = mask.unsqueeze(1)
            input_embed = self.decoder.embed_tokens(src_tokens)
            input_embed = self.dropout_layer(input_embed)
            if self.avg_feature:
                # src_outputs = (src_outputs[:, 51:] + input_embed) / 2
                src_outputs = (src_outputs[:, -src_tokens.size(-1):] + input_embed) / 2
            # word_scores = torch.einsum('blh,bnh->bln', hidden_state, src_outputs[:, 51:])  # bsz x max_len x max_word_len
            word_scores = torch.einsum('blh,bnh->bln', hidden_state, src_outputs[:, -src_tokens.size(-1):])
