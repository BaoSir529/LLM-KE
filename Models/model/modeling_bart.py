...
BART_PRETRAINED_MODEL_ARCHIVE_LIST = ["facebook/bart-base",
                                      "facebook/bart-large",
...
class PretrainedBartModel(PreTrainedModel):
    ...
    def dummy_inputs(self):
        ...
        dummy_inputs = {"attention_mask": input_ids.ne(pad_token),
                        "input_ids": input_ids}
        return dummy_inputs
...
class BartEncoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens):
        ...
        if config.static_position_embeddings:
            ...
            self.embed_positions = LearnedPositionalEmbedding(config.max_position_embeddings,
                                                              embed_dim,
                                                              self.padding_idx,
...
class DecoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        return (x,
                self_attn_weights,
                layer_state)  # just self_attn weights for now, following t5, layer_state = cache for decoding
...
class BartDecoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens: nn.Embedding):
        ...
        self.embed_tokens = embed_tokens
        if config.static_position_embeddings:
            self.embed_positions = SinusoidalPositionalEmbedding(config.max_position_embeddings,
                                                                 config.d_model,
                                                                 config.pad_token_id)
        else:
            self.embed_positions = LearnedPositionalEmbedding(config.max_position_embeddings,
    ...
    def set_position_embedding(self, special_tag_start_id, tag_first=True):
        if tag_first:
            embed_positions = DecoderLearnedPositionalEmbedding(self.config.max_position_embeddings,
                                                                self.config.d_model,
                                                                self.padding_idx,
                                                                self.config.extra_pos_embeddings,
                                                                special_tag_start_id)
        else:
            embed_positions = DecoderLearnedPositionalEmbedding2(self.config.max_position_embeddings,
                                                                 self.config.d_model,
                                                                 self.padding_idx,
                                                                 self.config.extra_pos_embeddings,
                                                                 special_tag_start_id)
    ...
    def forward(self,
        ...
        if "decoder_cached_states" in unused:
            warnings.warn("The `decoder_cached_states` argument is deprecated and will be removed in a future version, use `past_key_values` instead.",
                          FutureWarning)
            past_key_values = unused.pop("decoder_cached_states")
        if "decoder_past_key_values" in unused:
            warnings.warn("The `decoder_past_key_values` argument is deprecated and will be removed in a future version, use `past_key_values` instead.",
                          FutureWarning)
...
class Attention(nn.Module):
    ...
    def forward(self,
                ...
                output_attentions=False) -> Tuple[Tensor, Optional[Tensor]]:
    ...
    def forward(self, input_ids, use_cache=False):
        ...
        if self.special_tag_start_id is None or input_ids.size(1) < 2:
            ...
            if special_tag_mask.sum() > 0:
                ...
                _, inverted_indices = special_tag_mask.flip(dims=[1]).cumsum(dim=-1).flip(dims=[1]).unique(return_inverse=True)
...
class BartModel(PretrainedBartModel):
    ...
    def forward(
        ...
        decoder_outputs = self.decoder(decoder_input_ids,
                                        encoder_outputs[0],
                                        attention_mask,
                                        decoder_padding_mask,
